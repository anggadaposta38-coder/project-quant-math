...
import {
  applyHiddenSceneMotion,
  createHiddenSceneParticles,
  stepHiddenSceneMotion,
  type MotionParticle,
} from "@/lib/viz/motion-engine-v2";
...
function isHiddenStateScene(scene: Scene3D) {
  return (
    scene.axisLabels?.[0] === "Return" &&
    scene.axisLabels?.[1] === "Volatilitas" &&
    scene.axisLabels?.[2] === "Waktu"
  );
}
...
export const Plot3D = memo(function Plot3D({
  ...
  const hiddenParticlesRef = useRef<MotionParticle[]>([]);
  ...
  useEffect(() => {
    ...
    if (isHiddenStateScene(scene)) {
      hiddenParticlesRef.current =
        createHiddenSceneParticles(scene);
    }
  }, [scene]);
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      let renderableScene = sceneRef.current;

      if (isHiddenStateScene(renderableScene)) {
        hiddenParticlesRef.current =
          stepHiddenSceneMotion(
            hiddenParticlesRef.current,
            renderableScene,
        ...
        renderableScene = applyHiddenSceneMotion(
          renderableScene,
          hiddenParticlesRef.current,
        );
      } else {
        renderableScene = applyDataMotion(
          renderableScene,
          clock,
        );
      }
