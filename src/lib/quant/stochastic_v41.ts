...
import { annualizedVolatility, mean, mulberry32, normCdf, randNorm, shrinkageWeight, stdev } from "./stats";
...
export function monteCarloGbm(
  ...
  // Safe quantile for extreme crypto simulations. The generic quantile helper
  // can overflow in `a + (b - a) * fraction` when two finite prices are far
  // apart (b - a may become Infinity). A convex combination stays finite.
  const safeQuantile = (sorted: number[], prob: number): number => {
    ...
    const f = h - i;
    const value = a * (1 - f) + b * f;
    return Number.isFinite(value) ? value : Math.max(a, b);
  };

  const stableMeanPositive = (values: number[]): number => {
  ...
  const tailMean = stableMeanPositive(sortedT.slice(0, tailCount));
  ...
  return {
    ...
    bands,
    terminal,
    expectedTerminal: stableMeanPositive(terminal),
